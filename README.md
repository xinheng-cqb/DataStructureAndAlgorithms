...
其中前面三种树都属于二叉树，因此树的高度为(log2N)。树查找的时间复杂度与树的高度有关，因此要提高查找效率必须要降低树的高度。后面的两种是基于多叉树实现的。
### 二叉查找树（BST）
包名：structure.BST  
树的术语：  
...
待删除节点既有两个子节点时，分为两步删除：  
	1、找到待删除节点A的直接中根后继节点B（即该节点右子树中值最小的节点）,将B的右子树的值 赋给 B的父节点C的左子树，从树中删除B。   
	2、将B中的值赋给A （为啥这样做，百度找一下，这里只是描述代码逻辑,这块是这一模块的难点）  
...
平衡二叉树查找树的难点：    
 ...
 以上内容都搞定了，类基本上就写好了，然后就是各种调试把遗漏的条件或出错的地方改正。(针对上面的几点的展开内容，请自行百度)  
ps:我的代码里计算树高是在Node类里进行的，也可以在Tree类写个函数进行计算。插入和删除都测试了好多次，但是还是不保证会有bug，如果发现了，请指出，谢谢！  

### 红黑树（RBL）
包名：structure.RBL  
平衡二叉树查找树的难点：    
	1、增加和删除都会使得原先的红黑树失衡，怎么修正的问题是最难的        
	2、每个节点都需要关联父节点信息，以及子节点信息，还要带上颜色  
	3、旋转时同时要改变对应的父节点信息，不能遗漏
 	这里至少建议理解了二叉树后再来学这块的内容，而且内容挺绕的，参考url：http://blog.csdn.net/eson_15/article/details/51144079
...
·基数排序：基数排序   
### 冒泡排序
冒泡排序是通过两两比较，将小的数字后移，多次比较后来保证序列有序，比较简单，不多加描述
### 18大DM算法
包名 | 目录名 | 算法名 |
-----| ------ |--------|
